...
移行元サーバに接続できない・rsync が失敗した場合は、SSH のエラーメッセージと原因のヒントを表示します。

| 状況 | 終了コード | ヒント |
|---|---|:--|
| ファイアウォール等で接続不可 | 255 | SSH接続エラー。ファイアウォールがSSHポートをブロックしているか、移行元サーバに到達できません |
| 暗号アルゴリズム不一致 (`ssh-rsa` 無効化など) | 255 | 暗号アルゴリズム不一致。`-O "HostKeyAlgorithms=+ssh-rsa"` を追加してください |
| ネットワークI/Oエラー | 10 | 接続が拒否されたかネットワークに到達できません |
| 転送タイムアウト | 30 | ファイアウォールが通信を遮断している可能性があります |
| 接続タイムアウト | 35 | rsyncデーモンへの接続がタイムアウトしました |

表示例 (暗号アルゴリズム不一致の場合):

```
  エラー: 移行元サーバに接続できません (server1)
  Unable to negotiate with 192.0.2.10 port 22: no matching host key type found. Their offer: ssh-rsa
  ヒント: 暗号アルゴリズム不一致。-O "HostKeyAlgorithms=+ssh-rsa" を追加してください
```

SSH の生のエラーメッセージはログ (`result.log`) にも `[stderr]` 付きで記録されます。
